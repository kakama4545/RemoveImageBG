...
# Save Results
# (The save_output function remains unchanged.)
def save_output(image_name, output_name, pred, d_dir, type):
    predict = pred
    predict = predict.squeeze()
    predict_np = predict.cpu().data.numpy()
...
# Remove Background From Image (Generate Mask, and Final Results)
# Remove Background From Image (Generate Mask, and Final Results)
# (The removeBG function remains unchanged.)
...
@app.post('/removebg/')
async def removeBG(image_file: UploadFile = File(...), user: str = None):
    # ------- Load Trained Model --------
    print("---Loading Model---")
    model_name = 'u2net'
    ...
    if len(nparr) == 0:
        return JSONResponse(content={"message": "Empty image"}, status_code=400)

    # decode image
    try:
        img = cv2.imdecode(nparr, cv2.IMREAD_COLOR)
    except:
        return JSONResponse(content={"message": "Error decoding image"}, status_code=400)
    ...
    save_output(os.path.join(inputs_dir, unique_filename+'.jpg'), unique_filename+'.png', pred, results_dir, 'image')
    save_output(os.path.join(inputs_dir, unique_filename+'.jpg'), unique_filename+'.png', pred, masks_dir, 'mask')
